...
public partial class Hud : CanvasLayer
{
    ...
    private bool GhostFunction(ActionsPossible actionsPossible)
    {
        Sprite2D ghostImage = StoredData.CurrentSoldierNode.GhostSprite;
        ...
        {
            case ActionsPossible.MoveN: offset = new Vector2(0, -10); break;
            case ActionsPossible.MoveNE: offset = new Vector2(10, -10); break;
            case ActionsPossible.MoveE: offset = new Vector2(10, 0); break;
            case ActionsPossible.MoveSE: offset = new Vector2(10, 10); break;
            case ActionsPossible.MoveS: offset = new Vector2(0, 10); break;
            case ActionsPossible.MoveSW: offset = new Vector2(-10, 10); break;
            case ActionsPossible.MoveW: offset = new Vector2(-10, 0); break;
            case ActionsPossible.MoveNW: offset = new Vector2(-10, -10); break;
        ...
        // apply movement
        ghostImage.Position += offset;
        ghostCollisionShape.Position = ghostImage.Position;

        // collision test using the shape from the CollisionShape2D
        var shape = ghostCollisionShape.Shape;
        if (shape == null)
            return false;

        PhysicsShapeQueryParameters2D query = new PhysicsShapeQueryParameters2D();
        query.SetShape(shape);
        // set the shape transform to the ghost's global transform (or position)
        query.Transform = new Transform2D(0, ghostCollisionShape.GlobalPosition);
        // optional: exclude the current soldier so the ghost doesn't hit itself
        // Exclude the current soldier's Rid (if available)
        if (StoredData.CurrentSoldierNode != null && StoredData.CurrentSoldierNode.GhostCollisionShape != null)
        {
            var excludeShape = StoredData.CurrentSoldierNode.GhostCollisionShape.Shape;
            if (excludeShape != null)
            {
                query.Exclude = new Godot.Collections.Array<Godot.Rid> { excludeShape.GetRid() };
            }
        }

        var space = GetViewport().GetWorld2D().DirectSpaceState;
        var results = space.IntersectShape(query, 1); // maxResults=1 is enough for a boolean
